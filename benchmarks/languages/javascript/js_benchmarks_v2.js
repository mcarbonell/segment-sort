...
(function () {
    ...
    let blockMergeSegmentSortV2 = null;
    let blockMergeSegmentSortV3 = null;
    ...
    if (isNode) {
        ...
        blockMergeSegmentSortV2 = require('../../../implementations/javascript/block_merge_segment_sort_v2.js');
        blockMergeSegmentSortV3 = require('../../../implementations/javascript/block_merge_segment_sort_v3.js');
    ...
    const sorters = {
        ...
        blockMergeSegmentSortV2: (arr) => {
            const copy = [...arr];
            const start = process.hrtime.bigint();
            const sorted = blockMergeSegmentSortV2(copy);
            const end = process.hrtime.bigint();
            return { sorted, time: Number(end - start) / 1e6 };
        },

        blockMergeSegmentSortV3: (arr) => {
            const copy = [...arr];
            const start = process.hrtime.bigint();
            const sorted = blockMergeSegmentSortV3(copy);
            const end = process.hrtime.bigint();
            return { sorted, time: Number(end - start) / 1e6 };
        },
