...
/**
 * Runs all test cases against the blockMergeSegmentSort function.
 */
function runTests() {
    console.log('Running tests for Block Merge Segment Sort...\n');
    let passed = 0;
    let failed = 0;

    testCases.forEach((test, index) => {
        const input = [...test.input]; // Create a copy to avoid modifying the original test case
        const result = blockMergeSegmentSort(input);
